...
dataset_name = 'foo_063_rw0_nutrient_balance' #check
...
The data was downloaded manually from https://stats.oecd.org/viewhtml.aspx?datasetcode=AEI_NUTRIENTS&lang=en 
...
source = glob.glob(os.path.join(os.getenv("DOWNLOAD_DIR"),'AEI_NUTRIENTS_*.csv'))[0]
...
# subset the data to the nutrient balance indicators ('I0' - 'Nutrient inputs', 'OO' - 'Nutrient outputs, 'B0' - 'Balance (inputs minus outputs)' 'BPERHA' - 'Balance per hectare')
indicator_list = ['BPERHA', 'B0', 'I0', 'OO']
...
column_list =['iso', 'Country', 'Indicator', 'Nutrients', 'Time', 'Unit', 'Value']
df = df[column_list]
...
df['datetime'] = pd.to_datetime(df.time, format='%Y')
