...
dataset_name = 'ocn_006_projected_ocean_acidification'
...
land_zip = os.path.join(data_dir,os.path.basename(land_url))
urllib.request.urlretrieve(land_url, land_zip)
# unzip
with ZipFile(land_zip, 'r') as zip_ref:
...
for i in range(1,95):
    band_arg += '-b '+str(i) + ' '
cmd = 'gdal_rasterize {}-burn -128.0 {} {}'.format(band_arg,land_shp,processed_data_file)
logger.debug(cmd)
